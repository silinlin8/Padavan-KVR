...
INT STAInitialize(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (NdisEqualMemory(&pAd->EthConvert.EthCloneMac[0], &ZERO_MAC_ADDR[0], MAC_ADDR_LEN)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Read EEPROM, EthCloneMac is %02x:%02x:%02x:%02x:%02x:%02x!\n",
...
fds
				...
				if ((pApCliEntry->wdev.channel > 14) && (pMacEntry->MaxHTPhyMode.field.BW > 0) &&
					(RTMP_TIME_AFTER(pAd->Mlme.Now32, (pApCliEntry->ApcliInfStat.ApCliRcvBeaconTime_MlmeEnqueueForRecv) + (60 * OS_HZ)))) {
	...
	IN struct wifi_dev *wdev)
{
	...
	cached_idx = sta_search_pmkid_cache(pAd, paddr, if_index, wdev);
